...
src/
  ...
  lib/        доменная логика без React
    types.ts, format.ts, analytics.ts, recurring.ts, interest.ts, debts.ts,
    tasks.ts, seed.ts, icons.tsx
...
- **Проценты по депозиту считаются по остатку на каждый день**: ставка годовых
  / 365, отрицательный остаток ничего не приносит. Раз в месяц, в выбранное
  число, начисление появляется как доход «Проценты» и само становится частью
  остатка, так что следующий месяц считается уже с ним. Как и регулярные
  платежи, проценты начисляются при открытии приложения за все прошедшие
  месяцы, id собран из счёта и даты, поэтому второй раз не придут.
- **Долг, как и перевод, двигает только остаток счёта.** «Я дал» уменьшает
  счёт, возврат увеличивает; «Я взял» наоборот. Возвращать можно частями, долг
  закрыт, когда остаток дошёл до нуля. В «Всего на счетах» долги не входят.
...
`transfers`, долгам — `debts` и `debt_payments`, счетам — колонки `sort_order`
и `interest_*`. Если проект создавался до их появления, выполните
[supabase/schema.sql](supabase/schema.sql) ещё раз: скрипт идемпотентный. До
этого приложение открывается и работает, но не сохраняются задачи, переводы,
долги и изменения счетов.
